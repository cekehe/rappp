...
It is recommended to use the bead name structure Gene_Identifier. 
...
## CT
The CT element contains the raw MFI values for the coupling efficiency test in a data frame with replicates as rows and beads as columns.
Use same column naming as in the MFI element.
...
It's not really true for this mock data set, but in general there is a good correlation between the control beads and the medians, 
...
```{r results, fig.width=12, fig.height=5}
...
```{r key, fig.width=10, fig.height=5}
